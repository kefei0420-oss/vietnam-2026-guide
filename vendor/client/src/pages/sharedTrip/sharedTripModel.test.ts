...
import { formatDurationMinutes, isHttpUrl, linkHost, splitPhotoCredit, sharedPhotoUrl } from './sharedTripModel'
...
it('separates imported photo credits without losing notes and rejects unsafe image URLs', () => {
  expect(splitPhotoCredit('正文\n\n照片：作者 · CC BY\n来源：https://example.com')).toEqual({ note: '正文', credit: '照片：作者 · CC BY\n来源：https://example.com' })
  expect(splitPhotoCredit('普通\n\n段落')).toEqual({ note: '普通\n\n段落', credit: '' })
  expect(splitPhotoCredit(null)).toEqual({ note: '', credit: '' })
  expect(sharedPhotoUrl('/uploads/places/photo.jpg')).toBe('/uploads/places/photo.jpg')
  expect(sharedPhotoUrl('https://example.com/photo.jpg')).toBe('https://example.com/photo.jpg')
  expect(sharedPhotoUrl('javascript:alert(1)')).toBeNull()
  expect(sharedPhotoUrl('//example.com/photo.jpg')).toBeNull()
  expect(sharedPhotoUrl('/uploads/\\example.com')).toBeNull()
})
